...
| **MC floor fdv ≥ 10000** | 015_capital_efficiency_and_mc_structure.sql:73 | `new.mc_floor_ok := (new.fdv_usd >= 10000)` | fdv_usd | alertworthy (indirect via mc_structure) | mc_structure_ok requires fdv >= 10000 (88-91). |
...
| **is_alertworthy** | 015:112-115 | `(is_qualified is true) and (net_sol_inflow between 20 and 70) and (mc_structure_ok is true)` | is_qualified, net_sol_inflow, mc_structure_ok | alertworthy | Trigger only; not used for write filter in ingest. |
