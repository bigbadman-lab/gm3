-- mint_decision: add Option B fallback when mint not in v_paid_alertworthy_60 — try mint_any_latest (token_cache).
-- Signature and alertworthy behavior unchanged; not_found path tries token_cache and returns avoid/0.25 if found there.
...
begin
  ...
  if not found then
    -- Option B: try token_cache via mint_any_latest
    select * into tc from public.mint_any_latest(p_mint);
    if tc.found then
      return query select
        true,
        'avoid'::text,
        0.25::numeric,
        array[
          'Mint is tracked by GM3 (token_cache) but not present in the latest alertworthy snapshots.',
          'No alertworthy snapshot metrics available yet — treating as avoid by default.'
        ]::text[],
        null::uuid,
        null::int,
        p_mint,
    ...
    -- Not in token_cache either: existing not_found response
    return query select
      false,
      'not_found',
      0::numeric,
